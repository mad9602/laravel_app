$(function () {
    ...
    $('.toggle_wish').on('click', function () {
        ...
        $.ajax({
            ...
            data: { 'like_id': like_id, 'like_product': like_product, }, //コントローラーに送るに名称をつけてデータを指定
        })
            ...
            .done(function (data) //コントローラーからのリターンされた値をdataとして指定
            {
                if (data == 0) {
                    ...
                    click_button.children().attr("class", "bi-star");
                }
                if (data == 1) {
                    //クリックしたタグのステータスを変更
                    click_button.attr("like_product", "0");
                    //クリックしたタグの子の要素を変更(表示されているハートの模様を書き換える)
                    click_button.children().attr("class", "bi bi-star-fill");
                }
            })
            ////正常に処理が完了しなかった場合
            .fail(function (data) {
